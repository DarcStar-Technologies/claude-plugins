...
| `scripts/plan-kit-path.sh` | Run-time locator for the `plan-kit` provider's `validate-plan.sh` (the shared plan-shape gate): `$PLAN_KIT_DIR` → marketplace ancestor → `PATH`. The check itself is **not** vendored here — plan-kit owns it, and this template declares `plan-kit` as a dependency. |
