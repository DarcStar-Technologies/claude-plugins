...
#   cli     : {kind:"cli", name, versionProbe?:["--version"], versionPattern?:"<ERE>"}
...
for ((i = 0; i < count; i++)); do
  ...
  case "$kind" in
    cli)
      if command -v "$name" >/dev/null 2>&1; then
        ...
        if [[ -n "$pattern" ]]; then
          mapfile -t probe < <(jq -r '(.versionProbe // ["--version"])[]' <<<"$el")
          out="$("$name" "${probe[@]}" 2>&1 || true)"
    ...
    mcp)
      if command -v claude >/dev/null 2>&1; then
        if claude mcp list 2>/dev/null | grep -qiw -- "$name"; then
          status="OK"
    ...
    plugin)
      if [[ -f "$installed_json" ]]; then
        if jq -e --arg n "$name" \
          '(.plugins // {}) | keys | any(startswith($n + "@"))' "$installed_json" >/dev/null 2>&1; then
          status="OK"
          detail="installed"
        else
          status="MISSING"
  ...
  results="$(jq -c --argjson el "$el" --arg s "$status" --arg d "$detail" \
    '. + [$el + {status: $s, detail: $d}]' <<<"$results")"
done
