...
## [0.7.1](https://github.com/DarcStar-Technologies/claude-plugins/compare/plugin-forge--v0.7.0...plugin-forge--v0.7.1) (2026-07-18)


### Bug Fixes

* **plugin-forge:** `&`-safe, non-re-scanning render() + symlink-safe write ([#90](https://github.com/DarcStar-Technologies/claude-plugins/issues/90)) ([75ea672](https://github.com/DarcStar-Technologies/claude-plugins/commit/75ea672325ff4b7c5524961c1a5c80440d566d6a))
...
- `forge-scaffold.sh`'s `render()` corrupted scaffolded docs when the `--description`
  ...
  `{{DESC}}` by consuming the original string segment-by-segment (literal concatenation) — `&`-safe,
  and it never re-scans the inserted description (so a description that is itself the literal
  `{{DESC}}` still substitutes exactly once).
...
- `/forge`'s realize-the-plan step now spells out two shell-authoring pitfalls to avoid when writing a new plugin's scripts (don't re-scan inserted substitution text; guard file writes with physical `pwd -P` containment, not just lexical path checks).
