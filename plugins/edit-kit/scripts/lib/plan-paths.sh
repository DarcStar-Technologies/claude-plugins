#!/usr/bin/env bash
# plan-paths.sh — part of the edit-kit toolkit. Shared helpers for classifying the
# calling command's plan file paths. Sourced (never executed) by verify-repo.sh and
# scaffold-test.sh so both normalize a plan's files[] paths to the SAME
# target-relative form. Keeping this in one place stops the two from drifting — a
# divergence would let one scaffold a bundled test the other never runs, or vice versa.
