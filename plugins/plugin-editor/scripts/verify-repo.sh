#!/usr/bin/env bash
# verify-repo.sh — post-apply cross-checks for /edit-plugin. check-template.sh
# already hard-validates the edited plugin's own structure and shellchecks its
# scripts; this adds two checks that the edit didn't break things BEYOND the
# plugin's own files. Because /edit-plugin only ever edits inside the plugin dir,
# a failure that isn't fixable in-bounds must NOT falsely block a correct edit —
# so those parts are advisory:
#
