...
scripts/new-plugin.sh NAME --description "..."   # scaffold + register a plugin
...
- **`plugins/_template/`** — the reference plugin and the source `new-plugin.sh`
  copies. The `_` prefix marks it internal: validators skip `_`-prefixed plugins
  for marketplace membership, so it is never published.
- **`templates/*.tmpl`** — token files (`{{PLUGIN_NAME}}`, `{{PLUGIN_VERSION}}`,
  `{{DATE}}`, …) rendered by `new-plugin.sh`.
...
- **Registering a plugin is three files.** A new plugin must be added to
  ...
  `.release-please-manifest.json`. `new-plugin.sh` does all three with `jq` — use
...
- **Scaffold provenance is tracked.** Every public plugin has
  ...
  was generated from (written by `new-plugin.sh --template`, required by
...
- **`_template` is a copy source.** `new-plugin.sh` copies its component dirs and
